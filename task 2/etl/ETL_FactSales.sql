INSERT INTO `BI_OLAP_53`.`DM_FactSales`
SELECT
  S.`SalesOrderNumber`,
  ...
  SM.`Name`,
  SD.`UnitPrice`,
  @Discount:=(CASE WHEN PC.`Name`='Clothing' AND SD.`OrderQty` >= 5 AND SD.`OrderQty` < 10 THEN SD.`OrderQty` * SD.`UnitPrice` * 0.05
  ...
  END) AS `Discount`,
  SD.`OrderQty`,
  @LineTotal:=(SD.`OrderQty` * SD.`UnitPrice` - @Discount),
...
JOIN `BI_OLTP_53`.`TB_ShipMethod` AS SM
    ON S.ShipMethodID = SM.ShipMethodID
JOIN `BI_OLTP_53`.`TB_SalesOrderDetail` AS SD
    ON S.SalesOrderID = SD.SalesOrderID
JOIN `BI_OLTP_53`.`TB_Product` AS P
    ON SD.ProductID = P.ProductID
JOIN `BI_OLTP_53`.`TB_ProductCategory` AS PC
    ON P.ProductCategoryID = PC.ProductCategoryID
JOIN `BI_OLTP_53`.`TB_Address` AS A
